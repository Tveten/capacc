...
grid_plot_cpt_est <- function(plot_func,
  ...
  grid_plot(plots, dims, make_title(c(data, method), vars_in_title, "cpt"))
}
...
cpt_est_setup <- function(p = 10, precision_type = "banded",
  ...
  data <- init_data(n = n, p = p, precision_type = precision_type, band = band,
                    locations = locations, durations = durations)
  method <- method_params()
...
#' @export
plot_cpt_ests <- function(plot_func = boxplot_cpt_est, p = 10,
